...
Using \textbf{D-separation} \cite{bishop_pattern_2016}, \cite{ProbabilisticGraphicalModels}, \cite{ProbabilisticMachineLearning} on the \gls{dag} to simplify the general \gls{dvae} expressions \ref{gen_model_dvae} and \ref{inf_model_dvae}. Conditioning on $z_t$ and $z_{t-1}$ drives:
\begin{align}
...
\begin{frame}{DKF - Toy training - Loss}
    Posterior collapse... Need $\beta$ schedule (weight between reconstruction loss and $\mathbb{KL}$)
    \begin{center}
...
\begin{frame}{DKF - Toy training - Generations}
    \begin{center}
    \includegraphics[width=0.8\textwidth]{/home/benjamin/Folders_Python/MVA/MVA_Stage/images/dkf_preds.png}
...
\begin{frame}{VRNN - Toy training - Generation}
    \begin{center}
    \includegraphics[width=0.8\textwidth]{/home/benjamin/Folders_Python/MVA/MVA_Stage/images/vrnn_preds.png}
...
\begin{frame}{Gaussian Process - Variational Auto Encoder}
    \begin{figure}[H]
        \centering
        % \includegraphics[width=0.5\linewidth]{}
    ...
    \begin{itemize}
        \item The thick black lines indicate all latent variables follow a Gaussian Process. 
        \item The joint distribution writes somehow differently from the one for \glspl{dvae}, as:
            \begin{align}
            \label{joint_gpvae}
